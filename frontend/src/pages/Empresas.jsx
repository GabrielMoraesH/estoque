...
function Empresas() {
  ...
  return <Layout><main className="page-shell empresas-page">
    ...
    <Panel as="form" className="empresas-form" onSubmit={submit}>
      ...
      <div className="empresas-form-actions">{editing && <Button type="button" variant="secondary" disabled={saving} onClick={() => { setEditing(null); setForm(EMPTY); }}>Cancelar</Button>}<Button type="submit" disabled={saving || !form.nome.trim() || (!editing && !form.codigo.trim())}>{saving ? 'Salvando...' : editing ? 'Salvar alteração' : 'Criar empresa'}</Button></div>
    </Panel>
    <Panel>
